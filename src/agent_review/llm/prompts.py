...
def build_review_point_second_review_prompt(report: ReviewReport) -> str:
    payload = {
        "document_name": report.file_info.document_name,
        "review_points": [item.to_dict() for item in report.review_points[:12]],
        "applicability_checks": [item.to_dict() for item in report.applicability_checks[:12]],
        "quality_gates": [item.to_dict() for item in report.quality_gates[:12]],
        "formal_adjudication": [item.to_dict() for item in report.formal_adjudication[:12]],
    }
    return f"""请根据以下结构化审查结果，输出 JSON：
