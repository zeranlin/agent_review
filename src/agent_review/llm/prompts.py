...
9. 对陌生行业或陌生采购品目，优先把问题映射到跨行业可复用的风险母题，例如：项目属性错配、评分相关性不足、评分量化不足、材料负担过重、刚性门槛、模板残留、金额口径异常、程序审慎性不足。
"""
...
5. 对证书、检测报告、财务指标，要尽量判断其属于“一般要求”“偏重要求”还是“投标阶段刚性负担”。
3. 每个动态任务最多给出 2 组关键信号，每组 2-5 个词。
...
2. 不要虚构新证据，也不要扩大结论。
3. 强度判断必须尽量稳定落在以下五类之一：一般要求、偏重要求、刚性门槛、裁量过大、证据不足。
4. 主证据判断要明确回答：当前 primary_quote 是否足以代表该审查点；如不足，应说明应降为辅助证据或需要重新选主证据。
...
{{
  "review_point_second_reviews": [
    {{
      ...
      "primary_evidence_judgment": "当前主证据是否准确、是否应降为辅助证据",
      "supporting_evidence_judgment": "哪些证据更像辅助背景或仍需补主证据",
