...
def evidence_supports_title(title: str, quote: str) -> bool:
    checks = {
        ...
        "采购方式适用理由不足": [["采购方式", "竞争性磋商", "竞争性谈判", "单一来源", "询价"]],
        "混合采购未拆分或包件划分依据不足": [["货物", "服务", "人工管护", "运维", "施工"], ["采购包", "不划分采购包", "不分包采购", "包件划分"]],
        "资格条件与评分因素重复设门槛": [["资格", "资质", "证书", "业绩", "人员"], ["评分", "得分", "分"]],
        "资格条件可能缺乏履约必要性或带有歧视性门槛": [["科技型中小企业", "高新技术企业", "纳税信用", "成立满", "投标人"]],
        "资格业绩要求可能存在地域限定、行业口径过窄或与评分重复": [["同类业绩", "业绩"], ["市", "省", "行业", "不得少于", "提供"]],
        "不得限定供应商组织形式": [["个体工商户", "其他组织形式", "组织形式"], ["不得参与", "不接受", "不得投标", "不得参加", "不允许"]],
        ...
        "体系认证证书不得要求特定认证范围": [["认证范围"], ["管理体系认证", "认证证书"], ["得分", "得", "评分", "分值", "满分", "分"]],
        "不得将准入类、行政许可类资格职业证书设置为评分项": [["许可证", "行政许可", "作业人员证书", "特种设备安全管理和作业人员证书"], ["得分", "得", "评分", "分值", "满分", "分"]],
        "服务项目价格分值权重低于10%": [["价格权重", "价格分值权重", "价格分权重"], ["%"]],
        "资产总额被设为评分因素": [["资产总额"], ["得分", "得", "评分", "分值", "满分", "分"]],
        "从业人员被设为评分因素": [["从业人员"], ["得分", "得", "评分", "分值", "满分", "分"]],
        "纳税额被设为评分因素": [["纳税额"], ["得分", "得", "评分", "分值", "满分", "分"]],
        "成立年限被设为评分因素": [["成立时间满", "成立年限", "成立满"], ["得分", "得", "评分", "分值", "满分", "分"]],
        "综合评分法价格分未采用低价优先法": [["价格分计算方法"], ["中间价优先法", "平均值作为评标基准价", "去掉最高价和最低价", "低价优先法"]],
        "特定资质或证书要求超必要限度": [["资质", "证书", "认证", "检测报告"]],
        "技术或服务要求可验证性不足": [["满足采购人要求", "按行业标准", "高质量完成", "优质服务", "由采购人认定"]],
        "疑似使用不存在的技术标准": [["GB/T", "标准", "规范"]],
        "技术参数区间说明不足": [["响应时间", "精度", "范围", "区间"], ["ms", "mm"]],
        "同一技术参数区间说明冲突": [["不允许正偏离", "不允许负偏离", "不得偏离", "不接受偏离", "≤"], ["±", "偏差", "允许"]],
        "技术要求存在主观描述": [["操作体验", "界面友好美观", "设计美观", "人体工程学设计", "良好的操作体验", "友好美观"]],
        "验收与付款/考核/满意度联动不当": [["付款", "支付", "尾款"], ["验收", "考核", "满意度"]],
        "履约保证金转质量保证金或长期无息占压": [["履约担保", "履约保证金", "质量保证金"], ["无息退还", "银行转账", "合同总价"]],
        "第三方检测费用无论结果均由中标人承担": [["第三方检测费用", "检测费用"], ["中标人承担", "无论检测结果是否合格"]],
        "采购人应当在收到发票后N个工作日内完成资金支付/采购人应当在收到发票后N个工作日或Y日内完成资金支付": [["收到发票后"], ["支付", "付款", "资金支付"], ["工作日", "日内"]],
        "服务合同履行期限不得超过36个月": [["合同履行期限", "服务期限", "服务期", "建设周期"], ["个月"]],
        "以预算金额比例设最低报价门槛": [["预算金额", "80%"], ["无效投标", "不得低于"]],
    ...
    if title == "采购人单方解释或决定条款" and "采购代理机构" in quote:
        return False
    if title == "采购方式适用理由不足" and any(token in quote for token in ["项目名称", "预算金额", "采购单位"]) and "采购方式" not in quote:
        return False
    if title == "资格条件与评分因素重复设门槛" and all(token not in quote for token in ["资格", "评分", "得分", "分"]):
        return False
    if title == "资格条件可能缺乏履约必要性或带有歧视性门槛" and all(
        token not in quote for token in ["科技型中小企业", "高新技术企业", "纳税信用", "成立满"]
    ):
        return False
    if title == "资格业绩要求可能存在地域限定、行业口径过窄或与评分重复" and all(
        token not in quote for token in ["同类业绩", "业绩", "行业", "市", "省"]
    ):
        return False
    if title == "证明材料来源可能被限定为特定机构或特定出具口径" and all(
        token not in quote for token in ["检测中心", "检测机构", "实验室", "税务部门", "出具"]
    ):
        return False
    if title == "履约保证金转质量保证金或长期无息占压" and all(
        token not in quote for token in ["履约担保", "履约保证金", "质量保证金", "无息退还"]
    ):
        return False
    if title == "第三方检测费用无论结果均由中标人承担" and all(
        token not in quote for token in ["第三方检测费用", "中标人承担", "无论检测结果是否合格"]
    ):
        return False
    if title == "以预算金额比例设最低报价门槛" and all(
        token not in quote for token in ["预算金额", "80%", "无效投标", "不得低于"]
    ):
        return False
    if title == "资格条件与政策适用口径可能自相矛盾" and all(
        token not in quote for token in ["非专门面向中小企业", "科技型中小企业", "中小企业"]
    ):
        return False
    ...
    if title == "资产总额被设为评分因素" and all(
        ...
        return False
    if title == "从业人员被设为评分因素" and all(
        token not in quote for token in ["从业人员", "得分", "得", "评分", "分值", "满分", "分"]
    ):
        return False
    if title == "纳税额被设为评分因素" and all(
        token not in quote for token in ["纳税额", "得分", "得", "评分", "分值", "满分", "分"]
    ):
        return False
    if title == "成立年限被设为评分因素" and all(
        token not in quote for token in ["成立时间满", "成立年限", "成立满", "得分", "得", "评分", "分值", "满分", "分"]
    ):
        return False
    if title == "综合评分法价格分未采用低价优先法" and all(
        token not in quote for token in ["价格分计算方法", "中间价优先法", "平均值作为评标基准价", "去掉最高价和最低价", "低价优先法"]
    ):
        return False
    if title == "疑似使用不存在的技术标准" and all(
        token not in quote for token in ["GB/T", "标准", "规范"]
    ):
        return False
    if title == "技术参数区间说明不足" and all(
        token not in quote for token in ["响应时间", "精度", "范围", "区间", "ms", "mm"]
    ):
        return False
    if title == "同一技术参数区间说明冲突" and all(
        token not in quote for token in ["不允许正偏离", "不允许负偏离", "不得偏离", "不接受偏离", "≤", "±", "偏差", "允许"]
    ):
        return False
    if title == "技术要求存在主观描述" and all(
        token not in quote for token in ["操作体验", "界面友好美观", "设计美观", "人体工程学设计", "良好的操作体验", "友好美观"]
    ):
        return False
